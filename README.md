...
*   **L2 缓存 (L2Cache0-L2Cache3)**：每个CPU核心通常配有专属的L2缓存。L2缓存通过一个特定的**端口 (port_id)** 连接到片上总线系统。
*   **L3 缓存 (L3Cache0-L3Cache3)**：作为末级缓存 (LLC)，L3通常被所有CPU核心共享（逻辑上）。在本项目模拟的配置中，L3缓存被分成了多个部分，每个部分也通过一个**端口 (port_id)** 连接到总线。
*   **DDR 内存控制器 (MemoryNode0)**：负责与主内存进行数据交换，它也通过一个**端口 (port_id)** 连接到总线。
